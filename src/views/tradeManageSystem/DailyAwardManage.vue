        ...
        <el-dialog title="奖品编辑" v-model="awardEditVisible" width="80%" :append-to-body="true"
            ...
            <el-table border class="table" ref="multipleTable" header-cell-class-name="table-header" :data="awardInfo">
                ...
                <el-table-column label="物品名称">
                    <template #default="scope">
                        <span v-if="scope.row.type == '1'">{{scope.row.itemchinese}} 品质：{{!scope.row.itemquality||scope.row.itemquality==""?"无":scope.row.itemquality}}</span>
        ...
        methods: {
            ...
            handleSelect(item) {
                ...
                if (this.awardData.type == "1") {
                    ...
                    this.awardData.itemicon = (this.itemNameCache[item.value].icon.Value === null ? this.itemNameCache[item.value].itemname : this.itemNameCache[item.value].icon.Value) + ".png";
