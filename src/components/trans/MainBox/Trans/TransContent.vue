  ...
  methods: {
    ...
    ready() {
      ...
      $(".buying>.box").on("click", "li", function () {	//点击选中的物品
        ...
        //将物品图标和名称渲染到页面
        if (res[0].translate == null) {
          $(".buying .window>section>header>.name").html(ctx.HandleItemName(res[0].itemname));
        } else {
      ...
      $(".buying>.box").on("mouseenter", "li", function () {	//鼠标移入显示物品详情
        ...
        if (res[0].translate == null) {
          $(".buying>.content>.name").text(ctx.HandleItemName(res[0].itemname));
        } else {
      ...
      $(".player-com").on("click", "li", function () {
        ...
        var xb2 = $(this).data("itemid2");		//获取物品在数组中的下标
        ...
        if (xb1 == self.playerinfo.gameentityid) {	//如果店主是当前登录的玩家（浏览自己的店铺）
          ...
          if (self.class1 == "玩家售卖区") {
            ...
            $(".player-order>.window>section>header>.msg>.name").text(ctx.HandleItemName(name));	//名称
            ...
            $(".player-order>.window>footer>.confirm").click(function () {
              ...
              //todo 购买调用接口
              var buyParam = {"id": "" + xb2 + "", "count": "" + numVal + ""};
            ...
            var res = self.requireItemsAll.filter((item) => {
              return item.id == xb2
            });
            var item = res[0];
            console.log(item);
            ...
            $("#alert>.alert>footer>.confirm").click(function () {	//确认向玩家供货
              //下面开始检测自己的仓库是否存在这个物品
              var buyParam = {"id": "" + xb2 + ""};
              axios.post("api/supplyItem", buyParam).then(res => {
                if (res.data.respCode === "1") {
